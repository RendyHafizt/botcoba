...
# Subscription plans
MONTHLY_PLANS = {
    "1_month": {"price": 25000, "duration": 30, "name": "1 Month"},
    "3_months": {"price": 40000, "duration": 90, "name": "3 Months"},
    "6_months": {"price": 60000, "duration": 180, "name": "6 Months"},
    "12_months": {"price": 80000, "duration": 365, "name": "12 Months"}
}

DOCUMENT_PLANS = {
    "1_doc": {"price": 2000, "documents": 1, "name": "1 Document"},
    "5_docs": {"price": 10000, "documents": 5, "name": "5 Documents"},
    "10_docs": {"price": 20000, "documents": 10, "name": "10 Documents"}
}

BANK_DETAILS = """🏦 For Pay And Exclude Filter
📍 Contact : @Hyr175"""
...
def create_monthly_plans_menu():
    ...
    for plan_id, plan_info in MONTHLY_PLANS.items():
        button_text = f"{plan_info['name']} - Rp.{plan_info['price']}"
...
def callback_query(call):
    ...
    if call.data == "monthly_plans":
        ...
        help_text = """ℹ️ <b>Cara menggunakan Bot:</b>

1️⃣ Pilih subscription plan
2️⃣ Lakukan pembayaran dengan contact @hyr15
3️⃣ Kirim foto pembayaran ke @hyr15
4️⃣ Tunggu Approval dari admin
...
def show_pending_requests(call):
    ...
    for request_id, request_data in pending_only.items():
        ...
        requests_text += f"💰 Rp.{request_data['price']}\n"
...
💰 <b>Price:</b> Rp.{request_data['price']}
